...
Pattern.prototype.generateBackground = function () {
	...
	var baseColor = color.rgb2hsl(color.hex2rgb(this.opts.baseColor));
	...
	this.svg.rect(0, 0, '100%', '100%', {
		fill: color.rgb2rgbString(color.hsl2rgb(baseColor))
	});
};
...
Pattern.prototype.geoTrianglesRotated = function () {
	var scale         = hexVal(this.hash, 0);
	var sideLength    = map(scale, 0, 15, 15, 80);
	var triangleWidth = sideLength / 2 * Math.sqrt(3);
	var triangle      = buildRotatedTriangleShape(sideLength, triangleWidth);
	var fill, i, opacity, rotation, styles, val, x, y;

	this.svg.setWidth(triangleWidth * 6);
	this.svg.setHeight(sideLength * 3);
